...
class TestTaskOperations(unittest.TestCase):
    ...
    def test_move_to_quarantine(self):
        ...
        video_path = os.path.join(source_dir, "quarantined.mkv")
        with open(video_path, 'w') as f:
            f.write("fake video")
        quarantine_dir = os.path.join(self.tmpdir, "quarantine_q")
        task = self._create_task(status="FAILED", source_path=video_path,
                                 source_filename="quarantined.mkv")
        self.tm.move_to_quarantine(task["task_id"], video_path, [], quarantine_dir)
        updated = db_module.get_task(self.conn, task["task_id"])
        self.assertEqual(updated["file_location"], "quarantine")
        self.assertTrue(os.path.exists(os.path.join(quarantine_dir, "quarantined.mkv")))
...
class TestConfigKeyAccess(unittest.TestCase):
    """测试配置键访问是否正确"""
    def test_quarantine_dir_from_source_dedup(self):
        config = {
            "source_dedup": {
                "enabled": True,
        ...
        quarantine_dir = config.get("source_dedup", {}).get("quarantine_dir", "")
        self.assertEqual(quarantine_dir, "/data/quarantine")

    def test_quarantine_dir_missing(self):
        config = {}
        quarantine_dir = config.get("source_dedup", {}).get("quarantine_dir", "")
        self.assertEqual(quarantine_dir, "")

    def test_quarantine_dir_old_style_returns_empty(self):
        config = {"quarantine_dir": "/old/path"}
        quarantine_dir = config.get("source_dedup", {}).get("quarantine_dir", "")
        self.assertEqual(quarantine_dir, "")
