...
| 字段 | 值 |
|------|-----|
| 描述 | 当前 `process_one()` 没有记录"当前执行到哪个步骤"。如果进程在 copy→scrape 之间被 kill，重启后无法知道应该从哪一步恢复 |
| 影响 | 断点恢复测试（阶段四）只能验证"任务状态自洽"，无法验证"从断点恢复继续" |
| 严重程度 | 中（当前行为是重新处理，不丢失数据但浪费已完成的步骤） |
| 修复建议 | 在 task 记录中增加 `current_step` 字段，每步完成后更新。重启后从 `current_step` 继续而非从头开始 |
| 影响范围 | `runner.py`、DB schema、task_manager |

### ARCH-02: temp 目录残片清理依赖 `_cleanup_temp_on_failure`

| 字段 | 值 |
|------|-----|
| 描述 | 复制过程中产生 `.copying` 临时文件，只在 `_cleanup_temp_on_failure` 中清理。如果进程被 SIGKILL，`.copying` 文件将永久残留 |
| 影响 | 长时间运行后 temp 目录可能积累大量残留文件 |
| 严重程度 | 低（不影响功能，但占用磁盘空间） |
| 修复建议 | 在服务启动时增加 temp 目录扫描，清理所有 `.copying` 文件和超过 24h 的 temp 文件 |
| 影响范围 | `runner.py` 启动逻辑 |
