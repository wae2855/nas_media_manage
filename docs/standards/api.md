...
- 手动刮削搜索最多返回 20 条，类型和语言必须显式校验；候选必须用 Provider ID 通过 `scrape-apply` 先验证再持久绑定。请求只负责入队并快速返回，实际刮削、标准命名和文件处理必须在共享并发槽内执行；冲突和兜底仍需再次人工确认。
- 电视剧关联套用必须先提供可取消选择的预览，并在 `scrape-apply` 时服务端重新验证 `related_task_ids`；前端传入 ID 不能绕过同目录、同剧名、唯一季集号、状态和冲突门禁。待确认项重新排队，排队项继承人工绑定，运行中项不可改写；逐任务结果必须结构化返回，不能伪装为全部成功。
