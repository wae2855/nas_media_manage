...
系统支持两种清理模式，通过配置 `source_cleaner.cleanup_mode` 控制：
...
系统支持三种合并策略，通过配置 `source_cleaner.merge_strategy` 控制：
...
├── Movie.mkv          ← 主视频文件 → 保留
├── Movie.srt          ← 字幕文件 → 保留
├── Movie.nfo          ← 元数据文件(同名) → 保留
├── Movie.jpg          ← 海报文件(同名) → 保留
├── Movie.sample.mkv   ← 样本文件 → 删除(AI)
├── readme.txt         ← 说明文件 → 删除(规则)
└── ad.png             ← 广告图片 → 删除(规则)
```

#### 示例 2：小视频检测

配置 `junk_video_max_size_mb: 50`：

```text
├── Movie_2024.mkv       (2.5GB)     → 保留
├── Movie_trailer.mkv    (30MB)      → 删除(规则)
└── Sample.mp4           (10MB)      → 删除(规则)
```
