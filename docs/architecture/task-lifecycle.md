...
`organization_status` 是成功结果的后续整理状态，不是第二套任务终态：`FALLBACK_PENDING` 表示已成功入库到待整理区，`ORGANIZED` 表示关联新任务已完成规则整理或人工位置调整。两者都不得把原 `SUCCESS/DONE` 任务重新变回活动态。人工调整另建 `REORGANIZE` 子任务，原任务只同步当前影片/字幕路径和最近子任务 ID。

普通失败任务的 `retry` 回到 `PENDING/QUEUED` 并从来源重启；`REORGANIZE` 失败任务使用独立 `retry_reorganization` 回到 `PENDING/AWAIT_REVIEW`，保留片库来源、目标目录、文件名和人工意图，避免被普通来源流水线领取。
...
兜底确认也是受限子状态：普通入库任务必须显式接受“放入待整理区”；规则型重新整理任务仍匹配兜底时不展示确认按钮。所有仍有现存片库文件的已完成任务都可创建独立 `REORGANIZE` 子任务；人工指定目录的子任务固定停在 `AWAIT_REVIEW` 供用户核对原位置与目标位置，确认后才移动，原任务不重新进入活动态。
