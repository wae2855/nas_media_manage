...
`MetadataScraper` 是 pipeline 使用的刮削门面。它协调文件名清洗、Provider 查询、LLM 结构化和置信度计算。

`LLMScraper` 负责 LLM 调用和提示词装配。提示词文件由配置目录加载，Provider-specific prompts 由相关 API handler 维护。

`ConfidenceEngine` 负责把刮削结果、搜索置信度和数据来源门控转换为置信度等级。pipeline 的最终审核动作由 `ReviewDecisionService` 决定。
