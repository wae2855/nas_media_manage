...
| Task Detail & Actions | tasks modal/actions, `js/tasks.js`, `js/confidence-detail.js` | `GET /tasks/{id}`, `POST /tasks/{id}/retry`, `POST /tasks/{id}/confirm`, `POST /tasks/{id}/reclassify`, `POST /tasks/{id}/ignore`, `POST /tasks/{id}/rename`, `POST /tasks/{id}/delete`, `GET /tasks/{id}/subtitles`, `GET /logs` | `features.tasks`, `features.import_flow`, `features.scraping`, `features.recycle` | selected task, action loading, confidence detail modal, rename/reclassify draft | `tests/test_feature_task_delete.py`, `tests/test_task_operations.py` |
...
| Confidence Workspace | confidence section in config, `js/config.js`, `js/confidence-detail.js` | `GET /dimensions`, `GET /providers`, `POST /config/section` | `features.scraping`, `features.configuration` | confidence form state, dimension thresholds, AI consult prompt | confidence engine tests |
...
- `js/config.js`
Config shell + Import Rules + Providers & LLM + Confidence + Source Cleaner + Recycle。
