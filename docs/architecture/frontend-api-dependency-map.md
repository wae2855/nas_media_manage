...
| Matching & Scrape Preview | match section in config, `js/config.js` | `GET /dimensions`, `GET /providers`, `POST /scrape/preview`, `POST /config/section` | `features.scraping`, `features.configuration` | match form state, scrape preview two-level path | match engine tests |
