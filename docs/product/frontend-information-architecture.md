...
- 电视剧手动候选命中同剧同批次时，详情内先展示文件名、`SxxExx`、当前状态和处理方式；安全项默认全选并允许取消，运行中项不可选且明确说明不会改写。提供“仅处理当前集”和“应用所选”两个出口；提交期间显示旋转进度、锁定按钮和复选框，完成后按排队、排队继承、处理中未改写、跳过和失败汇总。
- 任务批量栏用“批量重新识别”统一处理失败与待确认任务，确认文案说明将使用当前版本规则重新刮削；运行中和已完成任务不进入请求。
...
- 进阶设置中的最大并发任务数只允许整数 1–2，默认并推荐普通 NAS 使用 1；加载历史超限值和保存前都收敛到该范围，界面明确说明大文件并行的资源影响。
