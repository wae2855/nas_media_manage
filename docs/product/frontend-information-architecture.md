...
同剧批量提交若有部分失败/排除/运行中未改写，保留结果面板并逐集展示文件名或任务 ID、原因和下一步。成功项不重复提交；刷新列表失败与提交失败分开提示，面板提供明确的“关闭并刷新任务”。
...
任务卡的季集信息使用“第 N 季 / 第 N 集”业务文案，维度区过滤重复的 `season` / `episode` 技术键；详情编辑标签使用“季数 / 集数”。普通已完成任务提供“调整存放位置”，用户选择启用片库和相对子目录后先创建独立人工调整任务，详情明确展示原位置、目标位置和文件包，再由用户确认执行。
