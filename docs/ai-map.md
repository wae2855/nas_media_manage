...
| `media_importer/features/source_files/coverage.py` | 同快照历史重复任务的完整文件包证明、人工/重启保留门禁 | [features/source-files.md](features/source-files.md), [standards/safety.md](standards/safety.md) | `tests/test_source_cleanup_coverage.py` |
| `media_importer/features/tasks/` | 任务管理、状态、协作停止、来源处置、只删记录、首页摘要、手动 Provider 候选、同剧批量套用及关联重新整理任务 | [features/tasks.md](features/tasks.md) | `tests/test_task_*.py`、`tests/test_series_batch_scrape_apply.py`、`tests/test_dashboard_summary.py`、`tests/test_p1_preview_api.py` |
...
| `media_importer/features/import_flow/` | 入库流程：runner、steps、confirm、services、Pipeline 级共享并发门禁 | [features/import-flow.md](features/import-flow.md), [features/tasks.md](features/tasks.md) | `tests/test_feature_import_flow.py`、`tests/test_task_concurrency_limit.py` 等 |
