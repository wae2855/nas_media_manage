...
| 兜底入库/重新整理/人工调整位置 | `tests/test_task_organization.py`, `tests/test_task_organization_ui.py`, `tests/test_task_organization_browser_ui.py`, `tests/test_bundle_restart_recovery.py`, `tests/test_file_flow_matrix.py`, `tests/test_feature_task_review.py`；真实临时目录验证父任务不重开、规则整理、root ID + 相对子目录、路径穿越拒绝、当前路径同步、字幕整包移动、目标冲突零覆盖和提交前后重启恢复，并由真实浏览器覆盖人工调整位置完整客户旅程 |
