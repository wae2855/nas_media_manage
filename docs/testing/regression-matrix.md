...
| 刮削/匹配（两级） | `tests/test_match_engine.py`, `tests/test_media_identity_resolution_v2.py`, `tests/test_tier2_match_engine.py`, `tests/test_tier2_correct.py`, `tests/test_match_engine_keyword_loop.py`, `tests/test_match_pipeline_integration.py`, `tests/test_review_decision_v2.py`, `tests/test_title_matcher.py`, `tests/test_identity_evidence.py`, `tests/test_full_frontend_flow_matrix_browser_ui.py`；覆盖显式/NFO ID、附加内容 NFO 继承边界、episode ID 非 series ID、Season/BDMV 正常继承、ID 冲突与异常降级、技术/无效目录连续上溯、多文件标题候选完整裁决、Latin-only 重音折叠、候选差距保护、广告伴随视频、多影片合集及文件/目录冲突人工确认 |
