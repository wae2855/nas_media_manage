...
> 2026-09-02 修订：ADR-0022 已删除中心中转目录和旧中转恢复。本计划中所有目录拓扑与启动恢复验收均以“来源、回收、日志、资源、目标片库 + 目标侧任务私有暂存”的现行合同为准。
...
上一轮只收敛了正常去重与替换路径，但破坏性审计在隔离目录真实复现了多条旁路：旧 FPK 仍能自动质量替换；运行任务在入库后、标记成功前崩溃时，重启清理会直接删除片库文件；目录配置没有阻止片库与来源、回收相同或父子嵌套；任务删除 API 可移走 `file_location=import` 的文件；已入库任务还能直接重命名片库文件。这些路径使现有安装包和源码都不能兑现“片库只新增”的产品承诺。
...
- 多片库中的每一个目标根都与来源、回收、日志、资源目录以及其他目标根保持真实路径互斥；相同、父包含子或子包含父都无法保存，也无法绕过配置直接运行副作用。
- 启动恢复只处理数据库任务清单列出的目标侧任务临时文件；任务路径已经指向正式片库文件时只复核状态，不触碰文件。
...
在 `features/configuration` 建立 canonical role topology 校验：所有路径先做绝对路径、`realpath` 和 `commonpath`；任意片库根与来源、回收、日志、资源目录相同或互为父子时返回结构化错误。多片库根之间也不得相同或互相包含。配置校验、保存、storage/startup readiness 和所有破坏性服务调用同一事实源，避免只靠前端或只在保存时拦截。

### 2. 启动恢复失败关闭

启动恢复不再根据任务状态直接删除 `task.video_path`。提交前仅允许删除数据库清单明确列出、位于目标根且满足任务临时后缀的普通文件；字幕使用同一规则。中断任务若已经保存了正式片库路径，只复核任务状态，不把片库文件改回来源语义；清单歧义时保留全部文件并要求人工检查。
...
- 严格遵守 [Safety Standards](../standards/safety.md)：源文件和片库正式文件禁止直接 `os.remove()`；只允许任务清单明确归属且位于目标根内的 `.tmp/.copying` 直接删除。
...
| 启动清理过度保守导致目标临时文件残留 | 数据安全优先；只保留无法证明归属的文件，并在日志/配置检查提示人工处理 |
...
- [x] 在启动任务恢复中使用任务清单、真实路径、安全普通文件、目标根与临时后缀门禁。
...
1. 片库与来源、回收、日志、资源及其他片库根相同或互为父子时，配置保存失败且所有文件副作用入口失败关闭。
2. `PENDING/RUNNING` 任务的 `video_path` 指向片库时，服务重启不会删除、移动或改名该文件；文件内容和指纹保持不变。
3. 目标任务临时清单越界、指向正式文件或经过符号链接逃逸时，启动清理数量为零；符号链接不能绕过真实路径检查。
...
- 启动恢复对未经任务清单、目标根、任务标识和临时后缀共同证明的路径调用直接删除。
