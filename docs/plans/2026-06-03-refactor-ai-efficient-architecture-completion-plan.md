...
- [x] 将任务 ignore 迁入 `features/tasks/file_lifecycle_service.py`，承接临时文件清理、字幕状态重置、回收站移交和 SKIPPED 状态更新。
- [x] 将 run-file 单文件处理入口迁入 `features/import_flow/run_file_service.py`，承接路径/扩展名校验、任务创建和后台处理启动。
- [x] 将 task detail/subtitles/stats 查询迁入 `features/tasks/detail_service.py`。
- [x] 将 run-batch 批处理入口迁入 `features/import_flow/run_file_service.py`。
- [x] 继续为其他高频 API 动作建立 feature service 或 application function。
- [x] 继续薄化其他 API handler，保留参数解析、错误包装、JSON 响应。
