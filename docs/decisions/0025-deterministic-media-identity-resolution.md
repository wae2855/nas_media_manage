...
2. 独立 NFO 身份读取器在视频相邻和作品根的受控候选中读取小型 XML，仅提取 TMDB/IMDb/TVDB ID；NFO 标题和年份只用于冲突校验，不扩散为模糊查询词。身份 scope 明确为 `movie/series/episode/unknown`，episode ID 不得当作 series ID；当前无 episode→series Provider 能力时保留证据并回退标题流程。
...
6. 标题使用统一 `TitleNormalizer`：严格模式执行 NFKC、casefold、撇号/破折号/标点/空白统一；宽松模式只对 Latin 基字符折叠重音，并做保守的 `&`/`and` 等价，不做音译或 NLP，也不改变非 Latin combining mark。
7. 同一文件的全部独立标题候选必须完成强匹配收集后才裁决：全部指向同一作品或其 Provider 官方别名可自动通过，指向不同作品必须人工确认；模糊候选继续按标题、年份、类型、季集、来源收敛等特征评分。热度只在业务证据同分时排序。第一、第二候选使用证据形态和 TitleMatcher 既有模糊边界共同判断差距，不采用脱离上下文的单一全局 magic number。
8. 路径角色分为 structural、generic、technical、supplementary、meaningful。结构/通用/技术目录可有限连续上溯；附加内容目录形成身份继承边界，禁止从作品根继承 `movie.nfo/tvshow.nfo`，但允许当前视频同 basename NFO。
