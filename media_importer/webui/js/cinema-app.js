...
function toggleCleanerPrompt(open) {
  ...
  const next =
    typeof open === "boolean" ? open : !editor.classList.contains("active");
...
async function loadDirectoryConfig() {
  ...
  setFieldValue(
    "cfg-llm_confidence_threshold",
    llm.confidence_threshold || 0.8,
  );
  document.getElementById("cfg-llm_verify_ssl").checked =
    llm.verify_ssl !== false;
  const scrapeModeSelect = document.getElementById("cfg-metadata_scrape_mode");
  if (scrapeModeSelect) {
    scrapeModeSelect.value =
      (rawConfig.metadata || {}).scrape_mode || "provider_first";
  ...
  setFieldValue(
    "cfg-source_cleaner-ai_prompt-inline",
    sourceCleaner.ai_prompt || "",
  );
  ...
  try {
    ...
    console.warn("loadTmdbPromptConfig 失败,继续后续同步", err);
  }
}
...
function bindEvents() {
  document.addEventListener("click", (event) => {
    ...
    if (event.target.closest("#btn-sc-prompt-close-inline"))
      toggleCleanerPrompt(false);
  ...
  document.addEventListener("input", (event) => {
    ...
    if (
      event.target.id === "cfg-llm_model" ||
      event.target.id === "cfg-llm_api_key" ||
      event.target.id === "cfg-llm_base_url" ||
      event.target.id === "cfg-llm_fast_model" ||
      event.target.id === "cfg-llm_fast_api_key" ||
      event.target.id === "cfg-llm_fast_base_url"
    ) {
      updateAiConfigStatus();
    }
  });
...
function updateAiConfigStatus() {
  const model = String(
    document.getElementById("cfg-llm_model")?.value || "",
  ).trim();
  const apiKey = String(
    document.getElementById("cfg-llm_api_key")?.value || "",
  ).trim();
  const baseUrl = String(
    document.getElementById("cfg-llm_base_url")?.value || "",
  ).trim();
  ...
  const fastModel = String(
    document.getElementById("cfg-llm_fast_model")?.value || "",
  ).trim();
  const fastApiKey = String(
    document.getElementById("cfg-llm_fast_api_key")?.value || "",
  ).trim();
  const fastBaseUrl = String(
    document.getElementById("cfg-llm_fast_base_url")?.value || "",
  ).trim();
  // AI assist uses its own API key first, falls back to scrape's
  const assistConfigured =
    fastModel && (fastApiKey || apiKey) && (fastBaseUrl || baseUrl);
  ...
  if (scrapeModeStatus) {
    const mode = String(
      document.getElementById("cfg-metadata_scrape_mode")?.value ||
        "provider_first",
    );
    const modeLabels = {
