...
class TestTaskManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        persistence_path = os.path.join(self.tmpdir, f"test_{id(self)}.json")
        self.config = {
            "source_dedup": {
                "enabled": True,
                "max_auto_retries": 3,
        ...
        self.tm = TaskManager(persistence_path, self.config)
    ...
    def test_15_check_source_duplicate_needs_review(self):
        t = self.tm.create_task(
            video_path="/dup/review.mkv", video_file="review.mkv"
        )
        db_module.update_task(self.tm.conn, t["task_id"], status="NEEDS_REVIEW")
        result = self.tm.check_source_duplicate("/dup/review.mkv")
        self.assertEqual(result["action"], "SKIP")
...
class TestFileScanner(unittest.TestCase):
    ...
    def test_12_scan_and_filter_with_dedup(self):
        ...
        tm = TaskManager(
            ...
            {"source_dedup": {"enabled": True, "max_auto_retries": 3}},
        )
        tm.conn = conn
        self.config["quarantine_dir"] = os.path.join(self.tmpdir, "quarantine")
...
class TestPipeline(unittest.TestCase):
    ...
    @patch('pipeline.FileCopier')
    @patch('pipeline.LLMScraper')
    def test_07_rollback_task(self, MockScraper, MockCopier, MockHooks):
        from pipeline import PipelineRunner

        video_path = self._create_video_file("RollbackTest.mkv")
        temp_copy = os.path.join(self.temp_dir, "RollbackTest.mkv")
        with open(temp_copy, 'w') as f:
            f.write("temp copy")
        ...
        with patch('pipeline.classify') as mock_classify:
            mock_classify.return_value = os.path.join(self.import_dir, "movies", "回退测试 (2024)")
            pipeline = PipelineRunner(
                config=self.config, task_manager=self.tm,
                metrics=None, logger=None, notifier=None,
            )
            task = self.tm.create_task(
                video_path=video_path, video_file="RollbackTest.mkv",
            )
            pipeline.process_one(task)
            self.assertEqual(task["status"], "CONFIRMING")

            result = pipeline.rollback_task(task["task_id"], self.source_dir)
            self.assertEqual(result["status"], "ROLLBACK")
            self.assertTrue(os.path.exists(os.path.join(self.source_dir, "RollbackTest.mkv")))
...
class TestAPIServer(unittest.TestCase):
    ...
    def test_07_task_rollback_db_update(self):
        t = db_module.create_task(self.conn, source_path="/rb/a.mkv", source_filename="a.mkv")
        db_module.update_task(self.conn, t["task_id"], status="CONFIRMING")
        db_module.update_task(self.conn, t["task_id"], status="ROLLBACK", error_message="用户回退到源目录")
        updated = db_module.get_task(self.conn, t["task_id"])
        self.assertEqual(updated["status"], "ROLLBACK")
...
class TestEndToEnd(unittest.TestCase):
    ...
    @patch('pipeline.FileCopier')
    @patch('pipeline.LLMScraper')
    def test_03_confirming_then_rollback(self, MockScraper, MockCopier, MockHooks):
        from pipeline import PipelineRunner

        video_path = self._create_source_file("Rollback.Movie.2024.mkv")
        temp_copy = os.path.join(self.temp_dir, "Rollback.Movie.2024.mkv")
        with open(temp_copy, 'w') as f:
            f.write("copied")
        ...
        with patch('pipeline.classify') as mock_classify:
            mock_classify.return_value = os.path.join(self.import_dir, "movies", "回退电影 (2024)")
            pipeline = PipelineRunner(
                config=self.config, task_manager=self.tm,
                metrics=None, logger=None, notifier=None,
            )
            task = self.tm.create_task(
                video_path=video_path, video_file="Rollback.Movie.2024.mkv",
            )
            pipeline.process_one(task)
            self.assertEqual(task["status"], "CONFIRMING")
            ...
            self.assertTrue(os.path.exists(os.path.join(self.source_dir, "Rollback.Movie.2024.mkv")))
    ...
    def test_05_source_dedup_quarantine_flow(self):
        t = db_module.create_task(
            self.conn,
        ...
        sub_path = os.path.join(self.source_dir, "old_movie.chs.srt")
        with open(sub_path, 'w') as f:
            f.write("subtitle")

        dedup = self.tm.check_source_duplicate(video_path)
        self.assertEqual(dedup["action"], "QUARANTINE")
        ...
        self.assertTrue(os.path.exists(os.path.join(self.quarantine_dir, "old_movie.mkv")))
        self.assertTrue(os.path.exists(os.path.join(self.quarantine_dir, "old_movie.chs.srt")))
        self.assertFalse(os.path.exists(video_path))
    ...
    def test_07_source_dedup_retry_flow(self):
        t = db_module.create_task(
            self.conn,
        ...
        self.assertEqual(dedup["action"], "RETRY")
        self.assertEqual(dedup["old_retry"], 1)
    ...
    def test_10_rollback_with_subtitles(self):
        video_path = self._create_source_file("sub_rollback.mkv")
        sub_path = os.path.join(self.source_dir, "sub_rollback.chs.srt")
        with open(sub_path, 'w') as f:
            f.write("subtitle")
        ...
        t = db_module.create_task(
            self.conn,
            source_path=video_path,
            source_filename="sub_rollback.mkv",
        )
        db_module.create_subtitles(self.conn, t["task_id"], [sub_path])
        db_module.update_task(self.conn, t["task_id"], status="CONFIRMING")

        temp_video = os.path.join(self.temp_dir, "sub_rollback.mkv")
        temp_sub = os.path.join(self.temp_dir, "sub_rollback.chs.srt")
        with open(temp_video, 'w') as f:
            f.write("temp video")
        with open(temp_sub, 'w') as f:
            f.write("temp sub")

        task = db_module.get_task(self.conn, t["task_id"])
        task["video_path"] = temp_video
        task["subtitle_files"] = [temp_sub]
        db_module.update_task(self.conn, t["task_id"], video_path=temp_video)

        from pipeline import PipelineRunner
        with patch('pipeline.HookRunner'), \
             patch('pipeline.FileCopier'), \
             patch('pipeline.LLMScraper'):
            pipeline = PipelineRunner(
                config=self.config, task_manager=self.tm,
            ...
            result = pipeline.rollback_task(t["task_id"], self.source_dir)
            self.assertEqual(result["status"], "ROLLBACK")

            self.assertTrue(os.path.exists(os.path.join(self.source_dir, "sub_rollback.mkv")))
            self.assertTrue(os.path.exists(os.path.join(self.source_dir, "sub_rollback.chs.srt")))
